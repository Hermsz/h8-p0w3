...
Contoh, [1, 3, 9, 27, 81] adalah deret aritmatika dengan pertambahan nilai sebesar pengalian 3, dan [1, 3, 9, 27, 48] bukanlah 
...
function tentukanDeretGeometri(arr) {
    ...
    for( var i = 0; i < arr.length - 1; i++ ) {
      arrTemp.push(Math.round(arr[i+1]/arr[i]))
    }

    for( var j = 1; j <= arrTemp.length - 1; j++) {
      if ( arrTemp[0] === arrTemp[j]) {
        hasil = true
      } else {
        hasil = false
      }
